...
import com.example.kakaoshop.product.response.OptionDTO;
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ...
        switch (id) {
            case 1 : {
                ...
                optionDTOList.add(new OptionDTO(1, "01. 슬라이딩 지퍼백 크리스마스에디션 4종", 10000));
                optionDTOList.add(new OptionDTO(2, "02. 슬라이딩 지퍼백 플라워에디션 5종", 10900));
                optionDTOList.add(new OptionDTO(3, "고무장갑 베이지 S(소형) 6팩", 9900));
                optionDTOList.add(new OptionDTO(4, "뽑아쓰는 키친타올 130매 12팩", 16900));
                optionDTOList.add(new OptionDTO(4, "2겹 식빵수세미 6매", 8900));
                responseDTO = new ProductRespFindByIdDTO(1, "기본에 슬라이딩 지퍼백 크리스마스/플라워에디션 에디션 외 주방용품 특가전", "", "/images/1.jpg", 1000, 5, optionDTOList);
                break;
            }
            case 2 : {
                List<OptionDTO> optionDTOList = new ArrayList<>();
                optionDTOList.add(new OptionDTO(6, "22년산 햇단밤 700g(한정판매)", 9900));
                optionDTOList.add(new OptionDTO(7, "22년산 햇단밤 1kg(한정판매)", 14500));
                optionDTOList.add(new OptionDTO(8, "밤깎기+다회용 구이판 세트", 5500));
